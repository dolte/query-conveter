...
function gfn_SetPopDivCalContent(str_val)
{
	...
	str_temp += 'TabOrder="60" Width="101">  ' + chr(10); 
				...
				for (var gi=0 ; gi < rCnt ; gi++ ){
					for (var gj=0 ; gj < cCnt ; gj++) {
						...
						if (Collect[i].GetCellProp("body",gj,"BKColor") =="1073741837") {
							...
							if (length(Collect[i].GetCellValue(gi,gj)) == 0){
								//gfn_SetStatusMsg(strMsg);
								gfn_Confirm(btn_style, strmsg, title, focus);
								Collect[i].setfocus();
								dsobj.row = gi;
...
function gfn_Excel2Grid(objGrid,strFileName,nSheet)
{
	...
	if(GV_MDIFG == "Y") {
		objFile = global.FrameTop.File_Global;
	} else {
		if(!find("File_Global")) {
	...
	if (objFile.IsExistFile(strFileName) == false)
	{
		var objFileDialog;
		if(GV_MDIFG == "Y") {
			objFileDialog = global.FrameTop.FileDialog_Global;
		} else {
