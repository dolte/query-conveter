...
public class tac10_r12_1
{
    ...
    /*
    */
    public DOBJ CTLsend_email_tmp2(DOBJ dobj)
    {
        ...
        try
        {
            dobj  = CALLsend_email_tmp2_XIUD8(Conn, dobj);           //  이메일데이터 임시데이터 삭제
            dobj  = CALLsend_email_tmp2_XIUD15(Conn, dobj);           //  이메일데이터 임시저장
            dobj  = CALLsend_email_tmp2_XIUD9(Conn, dobj);           //  문자발송데이터 임시데이터 삭제
            dobj  = CALLsend_email_tmp2_XIUD14(Conn, dobj);           //  문자발송데이터 임시저장
            dobj  = CALLsend_email_tmp2_XIUD10(Conn, dobj);           //  문자발송데이터 임시데이터 삭제
            dobj  = CALLsend_email_tmp2_SEL2(Conn, dobj);           //  발송대상 URL추출
            dobj  = CALLsend_email_tmp2_MPD1(Conn, dobj);           //  MMS저장LOOP
    ...
    public DOBJ CTLsend_email_tmp2( DOBJ dobj, Object Connx) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
    ...
    // 이메일데이터 임시데이터 삭제
    public DOBJ CALLsend_email_tmp2_XIUD8(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        while(dvobj.next())
        {
            sobj = SQLsend_email_tmp2_XIUD8(dobj, dvobj);
    ...
    private SQLObject SQLsend_email_tmp2_XIUD8(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
    ...
    // 이메일데이터 임시저장
    public DOBJ CALLsend_email_tmp2_XIUD15(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        QExecutor qexe = new QExecutor(dobj);
        ...
        while(dvobj.next())
        {
            sobj = SQLsend_email_tmp2_XIUD15(dobj, dvobj);
            updcnt += qexe.executeUpdate(Conn, sobj);
        }
        rvobj = new VOBJ();
        HashMap recordx = new HashMap();
    ...
    private SQLObject SQLsend_email_tmp2_XIUD15(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
    ...
    // 문자발송데이터 임시데이터 삭제
    public DOBJ CALLsend_email_tmp2_XIUD9(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        QExecutor qexe = new QExecutor(dobj);
        ...
        while(dvobj.next())
        {
            sobj = SQLsend_email_tmp2_XIUD9(dobj, dvobj);
            updcnt += qexe.executeUpdate(Conn, sobj);
        }
        rvobj = new VOBJ();
        HashMap recordx = new HashMap();
    ...
    private SQLObject SQLsend_email_tmp2_XIUD9(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        SQLObject sobj = new SQLObject();
        String    query="";
    ...
    // 문자발송데이터 임시저장
    public DOBJ CALLsend_email_tmp2_XIUD14(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        QExecutor qexe = new QExecutor(dobj);
        ...
        while(dvobj.next())
        {
            sobj = SQLsend_email_tmp2_XIUD14(dobj, dvobj);
            updcnt += qexe.executeUpdate(Conn, sobj);
        }
        rvobj = new VOBJ();
        HashMap recordx = new HashMap();
    ...
    private SQLObject SQLsend_email_tmp2_XIUD14(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        String   SUPP_YRMN = dobj.getRetObject("S").getRecord().get("SUPP_YRMN");   //지급 년월
        SQLObject sobj = new SQLObject();
    ...
    // 문자발송데이터 임시데이터 삭제
    public DOBJ CALLsend_email_tmp2_XIUD10(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        QExecutor qexe = new QExecutor(dobj);
        ...
        while(dvobj.next())
        {
            sobj = SQLsend_email_tmp2_XIUD10(dobj, dvobj);
            updcnt += qexe.executeUpdate(Conn, sobj);
        }
        rvobj = new VOBJ();
        HashMap recordx = new HashMap();
    ...
    private SQLObject SQLsend_email_tmp2_XIUD10(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        SQLObject sobj = new SQLObject();
        String    query="";
    ...
    // 발송대상 URL추출
    public DOBJ CALLsend_email_tmp2_SEL2(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        dobj.setRtnname("SEL2");
        VOBJ dvobj = dobj.getRetObject("S");           //사용자 화면에서 발생한 Object입니다.
        SQLObject  sobj = SQLsend_email_tmp2_SEL2(dobj, dvobj);
    ...
    private SQLObject SQLsend_email_tmp2_SEL2(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        SQLObject sobj = new SQLObject();
        String    query="";
    ...
    // MMS저장LOOP
    public DOBJ CALLsend_email_tmp2_MPD1(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        dobj.setRtnname("MPD1");
        VOBJ dvobj = dobj.getRetObject("SEL2");         //발송대상 URL추출에서 생성시킨 OBJECT입니다.(CALLsend_email_tmp2_SEL2)
        VOBJ       rvobj= null;
        dvobj.first();
        while(dvobj.next())
        {
            if(true)
            {
                ...
                dobj  = CALLsend_email_tmp2_OBJ1(Conn, dobj);           //  외부호출
                dobj  = CALLsend_email_tmp2_UPD16(Conn, dobj);           //  SMS정보 URL UPDATE
            }
        }
        return dobj;
    }
    // 외부호출
    public DOBJ CALLsend_email_tmp2_OBJ1(Connection Conn, DOBJ dobj) throws Exception
    {
    ...
    // SMS정보 URL UPDATE
    public DOBJ CALLsend_email_tmp2_UPD16(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        QExecutor qexe = new QExecutor(dobj);
        ...
        while(dvobj.next())
        {
            sobj = SQLsend_email_tmp2_UPD16(dobj, dvobj);
            updcnt += qexe.executeUpdate(Conn, sobj);
        }
        rvobj = new VOBJ();
        HashMap recordx = new HashMap();
    ...
    private SQLObject SQLsend_email_tmp2_UPD16(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
