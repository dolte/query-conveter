...
public class tac01_s02_2
{
    ...
    */
    ...
    private SQLObject SQLsearchGibu_bill_SEL1(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        ...
        String    query="";
        query +=" SELECT A.DEMD_NUM , A.BILL_NUM , A.ISS_DAY , A.BILL_KND , B.CODE_NM AS BILL_KND_NM , A.BILL_GBN , C.CODE_NM AS BILL_GBN_NM , A.BSCON_CD , A.BIPLC_GBN , A.BRAN_CD , A.BSCON_NM , A.SUPPPRES_NM , A.BSCON_INS_NUM , A.BSCON_FNM_NM , A.BSCON_REPPRES_NM , A.BSCON_POST_NUM , A.BSCON_ADDR , A.BSCON_BSCDTN , A.BSCON_BSTYP , A.SUPPPRES_CD , A.SUPPPRES_INS_NUM , A.SUPPPRES_FNM_NM , A.SUPPPRES_REPPRES_NM , A.SUPPPRES_ADDR , A.SUPPPRES_BSCDTN , A.SUPPPRES_BSTYP , A.SUPPTEMP_AMT , A.ATAX_AMT , A.REMAK , A.ISS_YN , 11 - LENGTH(A.SUPPTEMP_AMT) AS GONG_LEN , '1' AS EMAIL , NVL((SELECT APPTN_GBN  ";
        query +=" FROM GIBU.TBRA_BILL_ISS_MNG  ";
        query +=" WHERE BILL_NUM = A.BILL_NUM), '1') AS APPTN_GBN , DECODE(:BILL_KND, '3', D.EMAIL_ADDR, E.EMAIL_ADDR) EMAIL_ADDR , A.ELEC_SEND_YN  ";
        query +=" FROM FIDU.TTAC_BILL A , FIDU.TENV_CODE B , FIDU.TENV_CODE C , FIDU.TMEM_MB D , (SELECT BB.BILL_NUM, EMAIL_ADDR, BIOWN_NUM  ";
        query +=" FROM GIBU.TBRA_BILL_ISS_MST AA, GIBU.TBRA_BILL_ISS_DTL BB, GIBU.TBRA_UPSO CC  ";
        query +=" WHERE AA.UPSO_CD = CC.UPSO_CD  ";
        query +=" AND AA.DEMD_NUM = BB.DEMD_NUM) E  ";
        query +=" WHERE 1 = 1  ";
        ...
        query +=" AND A.BILL_NUM = E.BILL_NUM(+)  ";
        query +=" AND SUBSTR (A.ISS_DAY, 1, 6) LIKE SUBSTR(:DEMD_DAY, 1, 6) || '%'  ";
        query +=" AND A.BILL_KND = :BILL_KND  ";
        if( !BSCON_FNM_NM.equals("") )
        {
            query +=" AND A.BSCON_FNM_NM LIKE '%'|| :BSCON_FNM_NM ||'%'  ";
        }
        query +=" ORDER BY A.BILL_NUM DESC  ";
        sobj.setSql(query);
