...
public class bra04_r07_5
{
    ...
    /*
    */
    public DOBJ CTLnonpy_select_test2(DOBJ dobj)
    {
        ...
        try
        {
            ...
            {
                dobj  = CALLnonpy_select_test2_SEL1(Conn, dobj);           //  미징수 내역조회
                dobj  = CALLnonpy_select_test2_MRG7( dobj);        //  결과 합치기
            }
            else if( dobj.getRetObject("S").getRecord().get("UPSO_TYPE").equals("02"))
            {
                dobj  = CALLnonpy_select_test2_SEL5(Conn, dobj);           //  미징수 내역조회
                dobj  = CALLnonpy_select_test2_MRG7( dobj);        //  결과 합치기
            }
            else
            {
                dobj  = CALLnonpy_select_test2_SEL6(Conn, dobj);           //  미징수 내역조회
                dobj  = CALLnonpy_select_test2_MRG7( dobj);        //  결과 합치기
            }
        }
        catch(Exception e)
    ...
    public DOBJ CTLnonpy_select_test2( DOBJ dobj, Object Connx) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
    ...
    public DOBJ CALLnonpy_select_test2_SEL1(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        SQLObject  sobj = SQLnonpy_select_test2_SEL1(dobj, dvobj);
    ...
    private SQLObject SQLnonpy_select_test2_SEL1(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        query +=" AND  TO_CHAR(SYSDATE,  'YYYYMMDD'))  <  1  THEN  'N'  ELSE  'Y'  END)  ELSE  ''  END)  AS  VISIT_YN6  FROM  (   \n";
        ...
        query +=" SELECT  UPSO_CD  ,  GIBU.FT_GET_DEMD_AMT2(UPSO_CD,  MIN(YRMN),  :DEMD_YRMN,  :DEMD_YRMN,  'O')  DEMDS  FROM  (   \n";
        ...
        query +=" AND  A.UPSO_CD  =  B.UPSO_CD)  WHERE  B.NOTE_YRMN  IS  NULL  )  GROUP  BY  UPSO_CD  HAVING  COUNT(YRMN)  BETWEEN  TO_NUMBER(:START_CNT_MON)   \n";
        query +=" AND  TO_NUMBER(:END_CNT_MON)  +  1  )  )  XA  ,  GIBU.TBRA_UPSO  XB  ,  GIBU.TBRA_BSTYPGRAD  XC  ,  (   \n";
        query +=" SELECT  UPSO_CD  ,  MAX(GOSO_YN)  GOSO_YN  FROM  (   \n";
        query +=" SELECT  UPSO_CD  ,  DECODE(COMPN_DAY,  NULL,  '고소중',  DECODE(JUDG_CD,  '2',  '기소중지',  '4',  '구약식',  NULL))  GOSO_YN  FROM  GIBU.TBRA_ACCU  WHERE  (JUDG_CD  IN  ('2','4','10','11','12','13','14')   \n";
    ...
    // 결과 합치기
    // 조건에 맞는 결과 데이타를 내보낸다
    public DOBJ CALLnonpy_select_test2_MRG7(DOBJ dobj) throws Exception
    {
    ...
    public DOBJ CALLnonpy_select_test2_SEL5(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        dobj.setRtnname("SEL5");
        VOBJ dvobj = dobj.getRetObject("S");           //사용자 화면에서 발생한 Object입니다.
        SQLObject  sobj = SQLnonpy_select_test2_SEL5(dobj, dvobj);
        VOBJ rvobj = qexe.executeQuery(Conn, sobj);
        rvobj.setName("SEL5");
        rvobj.setRetcode(1);
        rvobj.Println("SEL5");
    ...
    private SQLObject SQLnonpy_select_test2_SEL5(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        double   START_ZIP = dvobj.getRecord().getDouble("START_ZIP");   //시작 우편번호
        double   START_CNT_MON = dvobj.getRecord().getDouble("START_CNT_MON");   //시작 월수
    ...
    // 미징수 내역조회
    // 모든 업소의 미징수 내역을 조회한다
    public DOBJ CALLnonpy_select_test2_SEL6(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        dobj.setRtnname("SEL6");
        VOBJ dvobj = dobj.getRetObject("S");           //사용자 화면에서 발생한 Object입니다.
        SQLObject  sobj = SQLnonpy_select_test2_SEL6(dobj, dvobj);
        VOBJ rvobj = qexe.executeQuery(Conn, sobj);
        rvobj.setName("SEL6");
        rvobj.setRetcode(1);
        rvobj.Println("SEL6");
    ...
    private SQLObject SQLnonpy_select_test2_SEL6(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        double   START_ZIP = dvobj.getRecord().getDouble("START_ZIP");   //시작 우편번호
        double   START_CNT_MON = dvobj.getRecord().getDouble("START_CNT_MON");   //시작 월수
