...
public class bra04_r04_3
{
    ...
    */
    ...
    private SQLObject SQLupso_rept_select_SEL1(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        ...
        SQLObject sobj = new SQLObject();
        String    query="";
        query +=" SELECT  XA.UPSO_CD  ,  XA.UPSO_NM  ,  XA.MNGEMSTR_NM  ,  XA.MNGEMSTR_RESINUM  ,  XA.BIOWN_NUM  ,  XA.MNGEMSTR_HPNUM  ,  XA.UPSO_NEW_ZIP  AS  UPSO_ZIP  ,  XA.UPSO_NEW_ADDR1  ||  DECODE(XA.UPSO_NEW_ADDR2,  '',  '',  ',  '  ||  XA.UPSO_NEW_ADDR2)  ||  XA.UPSO_REF_INFO  AS  ADDR  ,  XA.UPSO_PHON  ,  DECODE(XC.CNT,  0,  DECODE(NVL(GIBU.FT_GET_AUTO_CARD_USE(XA.UPSO_CD),'0'),'0','','카드'),  '은행')  AS  AUTO_YN  ,  XD.ACCU_YN  ,  (CASE  WHEN  GIBU.FT_GET_IS_BSCON(XA.UPSO_CD,  TO_CHAR(SYSDATE,  'YYYYMM'))  >  0  THEN  XB.MONPRNCFEE2  -  TRUNC(XB.MONPRNCFEE2  *   \n";
        query +=" (SELECT  MNG_RATE  /  100  FROM  GIBU.TBRA_BSCON_MNG_RATE  WHERE  BSTYP_CD  =  XB.BSTYP_CD))  ELSE  XB.MONPRNCFEE  END)  AS  MONPRNCFEE  ,  XB.MONPRNCFEE2  ,  XA.CLIENT_NUM  ,  XD.ACCU_DAY  ,  XD.ACCU_NUM  ,  XD.ACCU_BRAN  ,  XD.ACCU_GBN  ,   \n";
        query +=" (SELECT  COUNT(ACCU_DAY)  FROM  GIBU.TBRA_ACCU  WHERE  UPSO_CD  =  :UPSO_CD)  AS  ACCU_CNT  ,  :ONOFF_GBN  AS  ONOFF_GBN  ,   \n";
        query +=" (SELECT  DECODE(COUNT(*),  0,  '',  '채권의뢰')  FROM  GIBU.TBRA_DLGTN_CLAIM  WHERE  UPSO_CD  =  :UPSO_CD   \n";
        query +=" AND  COMPN_DAY  IS  NULL)  AS  CLAIM_GBN  ,   \n";
        query +=" (SELECT  NVL(MAX(APPTN_YRMN),  '')  AS  APPTN_YRMN  FROM  GIBU.TBRA_DLGTN_CLAIM  WHERE  UPSO_CD  =  :UPSO_CD   \n";
        query +=" AND  COMPN_DAY  IS  NULL)  AS  CLAIM_APPTN_YRMN  ,  DECODE(TRIM(XB.BSTYP_CD),  'o',  GIBU.FT_GET_NOREBANG_GRAD(XA.UPSO_CD),  XB.GRADNM)  AS  BSTYP_NM  ,   \n";
        query +=" (SELECT  IPPBX_INPHONE_NUM  FROM  FIDU.TENV_MEMBER  WHERE  USER_ID  =  XA.STAFF_CD)  AS  BRAN_PHON_NUM  ,   \n";
        query +=" (SELECT  DECODE(COL_MCH_YN,  '1',  '불가',  '')  FROM  GIBU.SDB_TBRA_UPSO  WHERE  UPSO_CD  =  XA.UPSO_CD)  AS  COL_MCH_YN  ,  (CASE  WHEN   \n";
        query +=" (SELECT  COUNT(1)  FROM  GIBU.TBRA_BSCON_CONTRINFO  WHERE  BSCON_CD  =  'T0000001'   \n";
        query +=" AND  USE_YN  =  'Y'   \n";
        query +=" AND  UPSO_CD  =  XA.UPSO_CD)  >  0   \n";
        query +=" OR  XB.BSTYP_CD  IN  ('k',  'l',  'o')  THEN  FLOOR(XB.MONPRNCFEE2  *   \n";
        query +=" (SELECT  MAX(MNG_RATE)  /  100  FROM  GIBU.TBRA_BSCON_MNG_RATE  WHERE  BSTYP_CD  =  XB.BSTYP_CD   \n";
        query +=" AND  APPL_DAY  <  TO_CHAR(SYSDATE,  'YYYYMMDD')))  END)  AS  KOSCAP_AMT  ,   \n";
        query +=" (SELECT  MONPRNCFEE  FROM  GIBU.TBRA_BSCON_CONTRINFO  WHERE  BSCON_CD  =  'T0000002'   \n";
        query +=" AND  USE_YN  =  'Y'   \n";
        query +=" AND  UPSO_CD  =  XA.UPSO_CD)  AS  FKMP_AMT  ,   \n";
        query +=" (SELECT  SUM(MONPRNCFEE)  FROM  GIBU.TBRA_BSCON_CONTRINFO  WHERE  BSCON_CD  =  'T0000003'   \n";
        query +=" AND  USE_YN  =  'Y'   \n";
        query +=" AND  UPSO_CD  =  XA.UPSO_CD)  AS  RIAK_AMT  ,   \n";
        query +=" (SELECT  BSCONHAN_NM  FROM  GIBU.TBRA_STOMU_CONTRINFO  A,  GIBU.TBRA_BSCON_STOMU  B  WHERE  A.UPSO_CD  =  XA.UPSO_CD   \n";
        query +=" AND  A.BSCON_CD  =  B.BSCON_CD   \n";
        query +=" AND  B.USE_YN  =  'Y')  AS  BSCON_NM  FROM  GIBU.TBRA_UPSO  XA  ,  (   \n";
        query +=" SELECT  ZA.UPSO_CD  ,  ZB.MONPRNCFEE  ,  ZB.MONPRNCFEE2  ,  ZC.GRADNM  ,  TRIM(ZB.BSTYP_CD)  AS  BSTYP_CD  ,  ZB.UPSO_GRAD  FROM  (   \n";
        ...
        query +=" SELECT  COUNT(*)  AS  CNT  FROM  GIBU.TBRA_UPSO_AUTO  WHERE  UPSO_CD  =  :UPSO_CD   \n";
        query +=" AND  TERM_YN  =  'N'  )  XC  ,  (   \n";
        query +=" SELECT  UPSO_CD  ,  ACCU_DAY  ,  ACCU_NUM  ,  ACCU_BRAN  ,  ACCU_GBN  ,  DECODE(CHK,  0,  DECODE(JUDG_CD,  '2',  '기소중지',  '3',  '기소유예',  '4',  '구약식',  '고소중'),  1,  DECODE(JUDG_CD,  '2',  '기소중지',  '4',  '구약식'),  '')  AS  ACCU_YN  FROM  (   \n";
        query +=" SELECT  UPSO_CD  ,  ACCU_DAY  ,  ACCU_NUM  ,  ACCU_BRAN  ,  ACCU_GBN  ,  DECODE(COMPN_DAY,  NULL,  0,  1)  AS  CHK  ,  JUDG_CD  FROM  GIBU.TBRA_ACCU  WHERE  UPSO_CD  =  :UPSO_CD  ORDER  BY  ACCU_DAY  DESC,  ACCU_NUM  DESC  )  WHERE  ROWNUM  =  1  )  XD  ,  INSA.TCPM_BIPLC  XF  WHERE  XA.UPSO_CD  =  :UPSO_CD   \n";
    ...
    // 원장조회
    // 조회년도를 기준으로 3개년도 입금 정보를 조회한다.  1. TENV_CODE 에서 입금구분의 명을 가져온다. 이때 입금구분의 HIGH_CD 값은 00147 이다. SQL 에서 HIGH_CD  값은 CODE_CD라는 컬럼명으로 정의해서 값을 설정한다  2018.02.27 이다섭 수정 원장에 [고소해결]이 제대로 뜨기 위해 고소 해결 건수를 세는 것에 JUDG_CD 추가(1 : 취하무입금, 41:구약식(해결) )
    ...
    private SQLObject SQLupso_rept_select_SEL4(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        ...
        query +=" AND  JUDG_CD  NOT  IN  ('2',  '3',  '4',  '8')  )  ACCU_SOLCNT  ,  D.ACCU_CNT  ,  A.USE_AMT  ,  A.REPT_DAY  ,  A.REPT_NUM  ,  A.RECV_DAY  ,  A.ACCU_GBN  ,  A.DISTR_SEQ  ,  A.MAPPING_DAY  ,  (CASE  WHEN  C.CODE_CD  =  '11'   \n";
        query +=" AND  E.REAS_GBN  =  'CR7'  THEN  '음악미사용'  ELSE  C.CODE_NM  END)  AS  CODE_NM  FROM  GIBU.TBRA_NOTE  A  LEFT  OUTER  JOIN  GIBU.TBRA_CONFIRM_DOC  E  ON  (A.UPSO_CD  =  E.UPSO_CD   \n";
        query +=" AND  TO_DATE(A.NOTE_YRMN,  'YYYYMM')  BETWEEN  E.START_DAY   \n";
        query +=" AND  E.END_DAY)  ,  FIDU.TENV_CODE  C  ,  (   \n";
    ...
    private SQLObject SQLupso_rept_select_SEL3(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        ...
        SQLObject sobj = new SQLObject();
        String    query="";
        query +=" SELECT  /*+  optimizer_features_enable('11.1.0.6')  */  B.UPSO_CD  ,  B.UPSO_NM  ,  B.MNGEMSTR_NM  ,  B.OPBI_DAY  ,  (   \n";
        ...
        query +=" AND  CODE_ETC  =  'A')  )  REPT_YRMN  ,  B.CLSBS_YRMN  ,  B.CLSBS_INS_DAY  ,   \n";
