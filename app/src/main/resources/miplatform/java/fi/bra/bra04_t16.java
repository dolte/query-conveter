...
public class bra04_t16
{
    ...
    */
    ...
    /*
    */
    public DOBJ CTLadj_del(DOBJ dobj)
    {
        WizUtil wutil = new WizUtil(dobj);
        Connection Conn = null;
        try
        ...
        try
        {
            dobj  = CALLadj_del_DEL6(Conn, dobj);           //  정정금액 삭제
            Conn.commit();
        }
        catch(Exception e)
        {
    ...
    public DOBJ CTLadj_del( DOBJ dobj, Object Connx) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        Connection Conn = (Connection)Connx;
        dobj  = CALLadj_del_DEL6(Conn, dobj);           //  정정금액 삭제
    ...
    // 정정금액 삭제
    public DOBJ CALLadj_del_DEL6(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        QExecutor qexe = new QExecutor(dobj);
        ...
        while(dvobj.next())
        {
            sobj = SQLadj_del_DEL6(dobj, dvobj);
            updcnt += qexe.executeUpdate(Conn, sobj);
        }
        rvobj = new VOBJ();
        HashMap recordx = new HashMap();
    ...
    private SQLObject SQLadj_del_DEL6(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        double   SEQ = dvobj.getRecord().getDouble("SEQ");   //관리번호
        String   UPSO_CD = dvobj.getRecord().get("UPSO_CD");   //업소 코드
        ...
        SQLObject sobj = new SQLObject();
        String    query="";
        query +=" Delete from GIBU.TBRA_MISU_ADJ  \n";
        query +=" where NONPY_DAY=:NONPY_DAY  \n";
        query +=" and UPSO_CD=:UPSO_CD  \n";
        query +=" and SEQ=:SEQ";
        sobj.setSql(query);
