...
public class bra01_s22_1
{
    ...
    /*
    */
    public DOBJ CTLget_acmcn_info(DOBJ dobj)
    {
        ...
        try
        {
            dobj  = CALLget_acmcn_info_SEL1(Conn, dobj);           //  업소별 반주기조회
            dobj  = CALLget_acmcn_info_SEL2(Conn, dobj);           //  업소의 기기총대수
        }
        catch(Exception e)
        {
            e.printStackTrace();
    ...
    public DOBJ CTLget_acmcn_info( DOBJ dobj, Object Connx) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
    ...
    // 업소별 반주기조회
    // 해당업소의 반주기 정보를 조회한다.
    public DOBJ CALLget_acmcn_info_SEL1(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        SQLObject  sobj = SQLget_acmcn_info_SEL1(dobj, dvobj);
    ...
    private SQLObject SQLget_acmcn_info_SEL1(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
    ...
    // 업소의 기기총대수
    public DOBJ CALLget_acmcn_info_SEL2(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        dobj.setRtnname("SEL2");
        VOBJ dvobj = dobj.getRetObject("S");           //사용자 화면에서 발생한 Object입니다.
        SQLObject  sobj = SQLget_acmcn_info_SEL2(dobj, dvobj);
        VOBJ rvobj = qexe.executeQuery(Conn, sobj);
        rvobj.setName("SEL2");
        dobj.setRetObject(rvobj);
        return dobj;
    }
    private SQLObject SQLget_acmcn_info_SEL2(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        String   UPSO_CD = dobj.getRetObject("S").getRecord().get("UPSO_CD");   //업소 코드
        SQLObject sobj = new SQLObject();
