...
public class bra01_s15_1
{
    ...
    */
    ...
    private SQLObject SQLupso_list_kylog_SEL1(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        ...
        String    query="";
        query +=" SELECT  INS_DATE  ,  UPSO_CD  ,  BRAN_CD  ,  BRAN_NM  ,  UPSO_NM  ,  GRAD_CD  ,  GRAD_NM  ,  MNGEMSTR_NM  ,  PERMMSTR_NM  ,  UPSO_ADDR  ,  DONG  ,  UPSO_NEW_ZIP  ,  UPSO_PHON  ,  MNGEMSTR_HPNUM  ,  ROOM_NAME  ,  SERIAL_NO  ,  BSCON_CD  ,  DECODE(SERIAL_NO,  NULL,  0,  1)  AS  SERIAL_GBN  ,  CO_STATUS_NM  ,  HYUPAEUP_GBN  ,  STAFF_CD  ,  DSCT_START  ,  DSCT_END  FROM  (   \n";
        ...
        query +=" (SELECT  --+  INDEX_DESC  (KDS_SHOPROOM  PK_KDS_SM_SEQ)  \n  ROOM_NAME  FROM  LOG.KDS_SHOPROOM  WHERE  UPSO_CD  =  A.UPSO_CD   \n";
        ...
        query +=" AND  CO_STATUS  =  '07001'   \n";
        query +=" AND  ROWNUM  =  1)  AS  BSCON_CD  ,  LOG.FT_GET_UPSO_STATUS_NM(A.UPSO_CD)  AS  CO_STATUS_NM  ,  GIBU.FT_GET_UPSO_HYUPAEUP(A.UPSO_CD)  AS  HYUPAEUP_GBN  ,  A.STAFF_CD  ,  SUBSTR(DSCT_START,1,4)  ||  '/'  ||  SUBSTR(DSCT_START,5,2)  AS  DSCT_START  ,  SUBSTR(DSCT_END,1,4)  ||  '/'  ||  SUBSTR(DSCT_END,5,2)  AS  DSCT_END  FROM  GIBU.TBRA_UPSO  A  ,  INSA.TCPM_DEPT  AA  ,  GIBU.TBRA_BSTYPGRAD  AAA  ,  LOG.KDS_SHOP  B  WHERE  A.BRAN_CD  =  AA.GIBU   \n";
