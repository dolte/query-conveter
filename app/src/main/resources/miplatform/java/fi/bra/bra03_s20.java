...
public class bra03_s20
{
    ...
    */
    ...
    /*
    */
    public DOBJ CTLauto_card_data_make(DOBJ dobj)
    {
        WizUtil wutil = new WizUtil(dobj);
        Connection Conn = null;
        try
        ...
        try
        {
            dobj  = CALLauto_card_data_make_OSP1(Conn, dobj);           //  청구 데이터 생성
            dobj  = CALLauto_card_data_make_SEL2(Conn, dobj);           //  청구 데이터 오류 내역
            dobj  = CALLauto_card_data_make_SEL3(Conn, dobj);           //  지부별 청구 완료 정보 조회
            Conn.commit();
        }
        catch(Exception e)
        {
    ...
    public DOBJ CTLauto_card_data_make( DOBJ dobj, Object Connx) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        Connection Conn = (Connection)Connx;
        dobj  = CALLauto_card_data_make_OSP1(Conn, dobj);           //  청구 데이터 생성
    ...
    public DOBJ CALLauto_card_data_make_OSP1(Connection Conn, DOBJ dobj) throws Exception
    {
    ...
    public DOBJ CALLauto_card_data_make_SEL2(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        dobj.setRtnname("SEL2");
        VOBJ dvobj = dobj.getRetObject("S");           //사용자 화면에서 발생한 Object입니다.
        SQLObject  sobj = SQLauto_card_data_make_SEL2(dobj, dvobj);
        VOBJ rvobj = qexe.executeQuery(Conn, sobj);
        rvobj.setName("SEL2");
        dobj.setRetObject(rvobj);
        return dobj;
    }
    private SQLObject SQLauto_card_data_make_SEL2(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        String   DEMD_YRMN = dobj.getRetObject("S").getRecord().get("DEMD_YRMN");   //청구 년월
        String   BRAN_CD = dobj.getRetObject("S").getRecord().get("BRAN_CD");   //지부 코드
    ...
    public DOBJ CALLauto_card_data_make_SEL3(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        dobj.setRtnname("SEL3");
        VOBJ dvobj = dobj.getRetObject("S");           //사용자 화면에서 발생한 Object입니다.
        SQLObject  sobj = SQLauto_card_data_make_SEL3(dobj, dvobj);
        VOBJ rvobj = qexe.executeQuery(Conn, sobj);
        rvobj.setName("SEL3");
        rvobj.setRetcode(1);
        dobj.setRetObject(rvobj);
    ...
    private SQLObject SQLauto_card_data_make_SEL3(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        String   DEMD_YRMN = dvobj.getRecord().get("DEMD_YRMN");   //청구 년월
        SQLObject sobj = new SQLObject();
