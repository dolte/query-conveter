...
public class bra03_s21
{
    ...
    */
    ...
    private SQLObject SQLcard_demd_select_XIUD2(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        ...
        String   PROC_GBN ="1";   //자동 처리 구분
        SQLObject sobj = new SQLObject();
        String    query="";
        query +=" UPDATE GIBU.TBRA_DEMD_CARD  \n";
        query +=" SET PROC_GBN= :PROC_GBN , MODPRES_ID = :MODPRES_ID , MOD_DATE = SYSDATE  \n";
        query +=" WHERE UPSO_CD IN (SELECT XB.UPSO_CD FROM GIBU.TBRA_DEMD_CARD XA , ( SELECT A.UPSO_CD , A.CARD_GBN , A.CARD_NUM , A.RESINUM FROM GIBU.TBRA_UPSO_AUTO A , ( SELECT UPSO_CD , MAX(AUTO_NUM) AUTO_NUM FROM GIBU.TBRA_UPSO_AUTO  \n";
