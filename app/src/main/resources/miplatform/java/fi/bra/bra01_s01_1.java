...
public class bra01_s01_1
{
    ...
    */
    public DOBJ CTLupsoinfo_search(DOBJ dobj)
    {
        ...
        try
        {
            dobj  = CALLupsoinfo_search_SEL6(Conn, dobj);           //  업소상세정보조회
            dobj  = CALLupsoinfo_search_SEL7(Conn, dobj);           //  자동이체 정보 조회
            if( dobj.getRetObject("SEL6").getRecordCnt() > 0 && dobj.getRetObject("SEL6").getRecord().get("BSTYP_CD").equals("o"))
            {
                dobj  = CALLupsoinfo_search_SEL4(Conn, dobj);           //  노래방상세
            }
        }
        catch(Exception e)
        {
    ...
    {
        WizUtil wutil = new WizUtil(dobj);
        String   CHG_BRAN = dobj.getRetObject("SEL6").getRecord().get("CHG_BRAN");   //변경 지부
        String   CHG_NUM = dobj.getRetObject("SEL6").getRecord().get("CHG_NUM");   //변경 번호
        String   CHG_DAY = dobj.getRetObject("SEL6").getRecord().get("CHG_DAY");   //변경 일자
        String   UPSO_CD = dobj.getRetObject("S").getRecord().get("UPSO_CD");   //업소 코드
        SQLObject sobj = new SQLObject();
        String    query="";
