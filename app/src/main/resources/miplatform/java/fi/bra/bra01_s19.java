...
public class bra01_s19
{
    ...
    */
    ...
    private SQLObject SQLhistory_search_SEL1(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        ...
        query +=" (SELECT  HAN_NM  FROM  INSA.TINS_MST01  WHERE  STAFF_NUM  =  A.INSPRES_ID)  AS  INSPRES_NM  ,  TO_CHAR(A.INS_DATE,  'YYYY-MM-DD  HH24:MI')  AS  INS_DATE  FROM  GIBU.TBRA_KYLOG_TRANS_LOG  A  ORDER  BY  A.YRMN  DESC ";
        sobj.setSql(query);
        return sobj;
    }
    //##**$$history_search
    ...
    /*
    */
    public DOBJ CTLdup_exec_check(DOBJ dobj)
    {
        WizUtil wutil = new WizUtil(dobj);
        Connection Conn = null;
        try
        ...
        try
        {
            dobj  = CALLdup_exec_check_SEL1(Conn, dobj);           //  중복실행 확인
        }
        catch(Exception e)
        {
            e.printStackTrace();
    ...
    public DOBJ CTLdup_exec_check( DOBJ dobj, Object Connx) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        Connection Conn = (Connection)Connx;
        dobj  = CALLdup_exec_check_SEL1(Conn, dobj);           //  중복실행 확인
    ...
    // 중복실행 확인
    public DOBJ CALLdup_exec_check_SEL1(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        dobj.setRtnname("SEL1");
        QExecutor qexe = new QExecutor(dobj);
        dobj.setRtnname("SEL1");
        VOBJ dvobj = dobj.getRetObject("S");           //사용자 화면에서 발생한 Object입니다.
        SQLObject  sobj = SQLdup_exec_check_SEL1(dobj, dvobj);
        VOBJ rvobj = qexe.executeQuery(Conn, sobj);
        rvobj.setName("SEL1");
        dobj.setRetObject(rvobj);
        return dobj;
    }
    private SQLObject SQLdup_exec_check_SEL1(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        SQLObject sobj = new SQLObject();
        String    query="";
        query +=" SELECT  COUNT(*)  AS  CNT  FROM  GIBU.TBRA_KYLOG_TRANS_LOG  A  WHERE  YRMN  BETWEEN  :START_YRMN   \n";
        query +=" AND  :END_YRMN ";
        sobj.setSql(query);
    ...
    */
    ...
    private SQLObject SQLlog_trans_exec_XIUD2(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        ...
        SQLObject sobj = new SQLObject();
        String    query="";
        query +=" INSERT INTO FIDU.TDIS_OFFUSETUNEINFO(UPSO_CD, USE_YRMN, SEQ, BSCON_CD, BSCON_PROD_CD, USE_START_DATE, USE_END_DATE, OCC_DAY, ACMCN_CD, USE_FREQ, INS_DATE, INSPRES_ID) SELECT UPSO_CD, USE_YRMN, ROWNUM SEQ, (SELECT --+ INDEX_DESC (KDS_SHOPROOM PK_KDS_SM_SEQ) \n BSCON_CD FROM LOG.KDS_SHOPROOM WHERE UPSO_CD = A.UPSO_CD AND CO_STATUS = '07001' AND ROWNUM = 1) AS BSCON_CD, BSCON_PROD_CD, USE_START_DATE, USE_END_DATE, OCC_DAY, ACMCN_CD, USE_FREQ, SYSDATE , :STAFF_CD FROM ( SELECT UPSO_CD, TO_CHAR (PLAY_SDATE, 'YYYYMM') USE_YRMN, SONG_ID BSCON_PROD_CD, PLAY_SDATE USE_START_DATE, PLAY_EDATE USE_END_DATE, TO_CHAR (PLAY_SDATE, 'YYYYMMDD') OCC_DAY, KARAOKE_NO ACMCN_CD, 1 USE_FREQ FROM LOG.KDS_STATISTICS WHERE TO_CHAR (PLAY_SDATE, 'YYYYMM') BETWEEN :START_YRMN AND :END_YRMN GROUP BY UPSO_CD, KARAOKE_NO, SONG_ID, PLAY_SDATE, PLAY_EDATE ) A";
        sobj.setSql(query);
        sobj.setString("END_YRMN", END_YRMN);               //종료년월
