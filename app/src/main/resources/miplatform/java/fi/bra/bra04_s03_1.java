...
public class bra04_s03_1
{
    ...
    */
    public DOBJ CTLrept_ocr_insert(DOBJ dobj)
    {
        ...
        try
        {
            ...
            if( dobj.getRetObject("SEL1").getRecord().get("BRANEND").equals("0"))
            {
                ...
                dobj  = CALLrept_ocr_insert_XIUD24(Conn, dobj);           //  통합징수 분 삭제
    ...
    // 통합징수 분 삭제
    public DOBJ CALLrept_ocr_insert_XIUD24(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        QExecutor qexe = new QExecutor(dobj);
        ...
        while(dvobj.next())
        {
            sobj = SQLrept_ocr_insert_XIUD24(dobj, dvobj);
            updcnt += qexe.executeUpdate(Conn, sobj);
        }
        rvobj = new VOBJ();
        HashMap recordx = new HashMap();
    ...
    private SQLObject SQLrept_ocr_insert_XIUD24(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        String   REPT_DAY = dvobj.getRecord().get("REPT_DAY");   //입금일자
        String   REPT_GBN = dvobj.getRecord().get("REPT_GBN");   //입금 구분
        SQLObject sobj = new SQLObject();
        String    query="";
        query +=" UPDATE GIBU.TBRA_DEMD_REPT_BSCON  \n";
        query +=" SET REPT_YRMN = '', REPT_DAY ='', REPT_NUM ='', MAPPING_YRMN='', MAPPING_DAY='', REPT_GBN='', REPT_AMT='', PROC_AMT=''  \n";
        query +=" WHERE REPT_DAY = :REPT_DAY  \n";
        query +=" AND REPT_GBN = :REPT_GBN";
        sobj.setSql(query);
        sobj.setString("REPT_DAY", REPT_DAY);               //입금일자
    ...
    public DOBJ CALLrept_ocr_insert_OSP1(Connection Conn, DOBJ dobj) throws Exception
    {
        ...
        String   spname ="GIBU.SP_PROC_REPT_OCR2";
