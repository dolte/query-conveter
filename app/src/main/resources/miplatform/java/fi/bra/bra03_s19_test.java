...
public class bra03_s19_test
{
    ...
    */
    public DOBJ CTLupload_test(DOBJ dobj)
    {
        ...
        try
        {
            dobj  = CALLupload_test_DEL1(Conn, dobj);           //  기존데이타삭제
            dobj  = CALLupload_test_XIUD16(Conn, dobj);           //  은행접수신청서삭제
            dobj  = CALLupload_test_MPD2(Conn, dobj);           //  건별처리
    ...
    public DOBJ CALLupload_test_MPD2(Connection Conn, DOBJ dobj) throws Exception
    {
        ...
        while(dvobj.next())
        {
            if(true)
            {
                ...
                dobj  = CALLupload_test_SEL70(Conn, dobj);           //  SEQ_NUM 획득
                dobj  = CALLupload_test_SEL3(Conn, dobj);           //  업소체크
                if( dobj.getRetObject("R").getRecord().get("APPTN_GBN").equals("01"))
                {
                    dobj  = CALLupload_test_INS42(Conn, dobj);           //  업소조회에러
                }
                ...
                {
                    dobj  = CALLupload_test_SEL13(Conn, dobj);           //  주민번호체크
                    ...
                    {
                        dobj  = CALLupload_test_INS15(Conn, dobj);           //  주민번호에러
                        dobj  = CALLupload_test_INS16(Conn, dobj);           //  신청서 등록
                    }
                    else
                    {
                        dobj  = CALLupload_test_SEL17(Conn, dobj);           //  은행정보체크
                        if( dobj.getRetObject("SEL17").getRecord().getDouble("CNT") > 0)
                        {
                            dobj  = CALLupload_test_INS19(Conn, dobj);           //  이력먼저 등록
                            dobj  = CALLupload_test_OSP20(Conn, dobj);           //  청구 데이터 생성
                        }
                        else
                        {
                            dobj  = CALLupload_test_INS21(Conn, dobj);           //  은행정보에러
                            dobj  = CALLupload_test_INS22(Conn, dobj);           //  신청서 등록
                        }
                    }
                }
                ...
                {
                    dobj  = CALLupload_test_INS45(Conn, dobj);           //  이력먼저 등록
                    dobj  = CALLupload_test_OSP46(Conn, dobj);           //  청구 데이터 생성
                }
                else
                {
                    dobj  = CALLupload_test_INS9(Conn, dobj);           //  업소조회에러
                }
            }
        }
        return dobj;
