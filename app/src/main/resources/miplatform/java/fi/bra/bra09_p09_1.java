...
public class bra09_p09_1
{
    ...
    /*
    */
    public DOBJ CTLstaff_collect_list(DOBJ dobj)
    {
        ...
        try
        {
            ...
            {
                dobj  = CALLstaff_collect_list_SEL1(Conn, dobj);           //  지역담당자징수현황(관리)
                dobj  = CALLstaff_collect_list_SEL2(Conn, dobj);           //  담당별 합계(관리)
                dobj  = CALLstaff_collect_list_MRG1( dobj);        //  리스트항목결과취합
                dobj  = CALLstaff_collect_list_MRG2( dobj);        //  담당별합계결과취합
            }
            else if( dobj.getRetObject("S").getRecord().get("GBN").equals("2"))
            {
                dobj  = CALLstaff_collect_list_SEL6(Conn, dobj);           //  지역담당자징수현황(개발)
                dobj  = CALLstaff_collect_list_SEL7(Conn, dobj);           //  담당별 합계(개발)
                dobj  = CALLstaff_collect_list_MRG1( dobj);        //  리스트항목결과취합
                dobj  = CALLstaff_collect_list_MRG2( dobj);        //  담당별합계결과취합
            }
            else
            {
                dobj  = CALLstaff_collect_list_SEL8(Conn, dobj);           //  지역담당자징수현황(전체)
                dobj  = CALLstaff_collect_list_SEL9(Conn, dobj);           //  담당별 합계(전체)
                dobj  = CALLstaff_collect_list_MRG1( dobj);        //  리스트항목결과취합
                dobj  = CALLstaff_collect_list_MRG2( dobj);        //  담당별합계결과취합
            }
        }
    ...
    public DOBJ CTLstaff_collect_list( DOBJ dobj, Object Connx) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
    ...
    // 지역담당자징수현황(관리)
    public DOBJ CALLstaff_collect_list_SEL1(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        SQLObject  sobj = SQLstaff_collect_list_SEL1(dobj, dvobj);
    ...
    private SQLObject SQLstaff_collect_list_SEL1(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        String    query="";
        query +=" SELECT  /*+  optimizer_features_enable('11.1.0.6')  */  BRAN_CD  ,   \n";
        ...
        query +=" AND  C.REPT_GBN(+)  =  '08'  )  B  ,  INSA.TINS_MST01  C  WHERE  A.BRAN_CD  =  DECODE(:BRAN_CD,  'AL',  BRAN_CD,  :BRAN_CD)   \n";
    ...
    // 담당별 합계(관리)
    public DOBJ CALLstaff_collect_list_SEL2(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        dobj.setRtnname("SEL2");
        VOBJ dvobj = dobj.getRetObject("S");           //사용자 화면에서 발생한 Object입니다.
        SQLObject  sobj = SQLstaff_collect_list_SEL2(dobj, dvobj);
        VOBJ rvobj = qexe.executeQuery(Conn, sobj);
        rvobj.setName("SEL2");
        dobj.setRetObject(rvobj);
        return dobj;
    }
    private SQLObject SQLstaff_collect_list_SEL2(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        String   YRMN = dobj.getRetObject("S").getRecord().get("YRMN");   //년월
        String   BRAN_CD = dobj.getRetObject("S").getRecord().get("BRAN_CD");   //지부 코드
        SQLObject sobj = new SQLObject();
        String    query="";
        query +=" SELECT  BRAN_CD  ,  SUM(AMT)  MON_AMT  ,  SUM(DECODE(GBN,  'UPSO',1,0))  UPSO_CNT  ,  SUM(NONPY_AMT)  NONPY_AMT  ,  SUM(DECODE(NONPY_AMT,0,0,1))  NONPY_CNT  ,  SUM(USE_AMT)  COL_AMT  ,  SUM(DECODE(GBN,  'COL_UPSO',1,0))  COL_UPSO_CNT  ,  STAFF_NM  ,  SUM(USE_AMT)  STAFF_AMT  FROM  (   \n";
    ...
    // 리스트항목결과취합
    public DOBJ CALLstaff_collect_list_MRG1(DOBJ dobj) throws Exception
    {
    ...
    // 담당별합계결과취합
    public DOBJ CALLstaff_collect_list_MRG2(DOBJ dobj) throws Exception
    {
        dobj.setRtnname("MRG2");
        WizUtil wutil = new WizUtil(dobj);
        VOBJ       rvobj= null;
        rvobj = wutil.getMergeObject(dobj,"SEL2, SEL7, SEL9","BRAN_CD, MON_AMT, UPSO_CNT, NONPY_AMT, NONPY_CNT, COL_AMT, COL_UPSO_CNT,  STAFF_NM, STAFF_AMT");
        rvobj.setName("MRG2") ;
        rvobj.setRetcode(1);
        dobj.setRetObject(rvobj);
        return dobj;
    ...
    // 지역담당자징수현황(개발)
    public DOBJ CALLstaff_collect_list_SEL6(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        dobj.setRtnname("SEL6");
        VOBJ dvobj = dobj.getRetObject("S");           //사용자 화면에서 발생한 Object입니다.
        SQLObject  sobj = SQLstaff_collect_list_SEL6(dobj, dvobj);
        VOBJ rvobj = qexe.executeQuery(Conn, sobj);
        rvobj.setName("SEL6");
        dobj.setRetObject(rvobj);
        return dobj;
    }
    private SQLObject SQLstaff_collect_list_SEL6(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        String   YRMN = dobj.getRetObject("S").getRecord().get("YRMN");   //년월
        String   BRAN_CD = dobj.getRetObject("S").getRecord().get("BRAN_CD");   //지부 코드
    ...
    // 담당별 합계(개발)
    public DOBJ CALLstaff_collect_list_SEL7(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        dobj.setRtnname("SEL7");
        VOBJ dvobj = dobj.getRetObject("S");           //사용자 화면에서 발생한 Object입니다.
        SQLObject  sobj = SQLstaff_collect_list_SEL7(dobj, dvobj);
        VOBJ rvobj = qexe.executeQuery(Conn, sobj);
        rvobj.setName("SEL7");
        dobj.setRetObject(rvobj);
        return dobj;
    }
    private SQLObject SQLstaff_collect_list_SEL7(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        String   YRMN = dobj.getRetObject("S").getRecord().get("YRMN");   //년월
        String   BRAN_CD = dobj.getRetObject("S").getRecord().get("BRAN_CD");   //지부 코드
        SQLObject sobj = new SQLObject();
        String    query="";
        query +=" SELECT  BRAN_CD  ,  SUM(AMT)  MON_AMT  ,  SUM(DECODE(GBN,  'UPSO',1,0))  UPSO_CNT  ,  SUM(NONPY_AMT)  NONPY_AMT  ,  SUM(DECODE(NONPY_AMT,0,0,1))  NONPY_CNT  ,  0  COL_AMT  ,  0  COL_UPSO_CNT  ,  STAFF_NM  ,  SUM(USE_AMT)  STAFF_AMT  FROM  (   \n";
        query +=" SELECT  XA.BRAN_CD  ,  XA.UPSO_CD  ,  MAX(XC.MONPRNCFEE)  AMT  ,  0  USE_AMT  ,  MAX(XD.STAFF_NUM)  STAFF_CD  ,  MAX(XD.HAN_NM)  STAFF_NM  ,  SUM(NVL(XB.TOT_DEMD_AMT,0))  NONPY_AMT  ,  'UPSO'  GBN  FROM  GIBU.TBRA_UPSO  XA  ,  (   \n";
        query +=" SELECT  UPSO_CD  ,  DEMD_YRMN  ,  TOT_USE_AMT+TOT_ADDT_AMT  TOT_DEMD_AMT  FROM  (   \n";
        query +=" SELECT  UPSO_CD  ,  :YRMN  AS  DEMD_YRMN  ,  GIBU.FT_SPLIT(DEMDS,  ',',  0)  START_YRMN  ,  GIBU.FT_SPLIT(DEMDS,  ',',  1)  END_YRMN  ,  GIBU.FT_SPLIT(DEMDS,  ',',  2)  BSTYP_CD  ,  GIBU.FT_SPLIT(DEMDS,  ',',  3)  UPSO_GRAD  ,  GIBU.FT_SPLIT(DEMDS,  ',',  4)  MONPRNCFEE  ,  GIBU.FT_SPLIT(DEMDS,  ',',  4)  DEMD_GBN  ,  GIBU.FT_SPLIT(DEMDS,  ',',  7)  DEMD_MMCNT  ,  GIBU.FT_SPLIT(DEMDS,  ',',  8)  TOT_USE_AMT  ,  GIBU.FT_SPLIT(DEMDS,  ',',  9)  +  GIBU.FT_SPLIT(DEMDS,  ',',  10)  TOT_ADDT_AMT  FROM  (   \n";
        query +=" SELECT  A.UPSO_CD  ,  GIBU.FT_GET_DEMD_AMT2(A.UPSO_CD,  A.START_YRMN,  :YRMN,  :YRMN,  'O')  DEMDS  FROM  (   \n";
    ...
    // 지역담당자징수현황(전체)
    public DOBJ CALLstaff_collect_list_SEL8(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        dobj.setRtnname("SEL8");
        VOBJ dvobj = dobj.getRetObject("S");           //사용자 화면에서 발생한 Object입니다.
        SQLObject  sobj = SQLstaff_collect_list_SEL8(dobj, dvobj);
        VOBJ rvobj = qexe.executeQuery(Conn, sobj);
        rvobj.setName("SEL8");
        dobj.setRetObject(rvobj);
        return dobj;
    }
    private SQLObject SQLstaff_collect_list_SEL8(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        String   YRMN = dobj.getRetObject("S").getRecord().get("YRMN");   //년월
        String   BRAN_CD = dobj.getRetObject("S").getRecord().get("BRAN_CD");   //지부 코드
    ...
    // 담당별 합계(전체)
    public DOBJ CALLstaff_collect_list_SEL9(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        dobj.setRtnname("SEL9");
        VOBJ dvobj = dobj.getRetObject("S");           //사용자 화면에서 발생한 Object입니다.
        SQLObject  sobj = SQLstaff_collect_list_SEL9(dobj, dvobj);
        VOBJ rvobj = qexe.executeSql(Conn, sobj);
        rvobj.setName("SEL9");
        dobj.setRetObject(rvobj);
        return dobj;
    }
    private SQLObject SQLstaff_collect_list_SEL9(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        String   YRMN = dobj.getRetObject("S").getRecord().get("YRMN");   //년월
        String   BRAN_CD = dobj.getRetObject("S").getRecord().get("BRAN_CD");   //지부 코드
