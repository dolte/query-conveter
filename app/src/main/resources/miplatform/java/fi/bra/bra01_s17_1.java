...
public class bra01_s17_1
{
    ...
    */
    ...
    private SQLObject SQLsearch_logcnt_stat_SEL1(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        ...
        String    query="";
        query +=" SELECT  /*+  optimizer_features_enable('11.1.0.6')  */  B.UPSO_CD,  B.SERIAL_NO,  NVL(A.CNT,0)  AS  CNT,   \n";
        query +=" (SELECT  CO_PARING  FROM  LOG.KDS_LOGCOLLECTOR  WHERE  SERIAL_NO  =  B.SERIAL_NO)  AS  CO_PARING,  B.BRAN_NM,  B.UPSO_NM,  B.BSTYP_NM,  B.STAFF_NM,  B.BSCON_CD  FROM  (   \n";
        ...
        query +=" (SELECT  A.SERIAL_NO,  A.UPSO_CD,  C.BRAN_CD,  C.UPSO_NM,  C.BRAN_NM,  C.BSTYP_NM,  C.STAFF_NM,  A.BSCON_CD  FROM   \n";
        query +=" (SELECT  SERIAL_NO,  SEQ,  UPSO_CD,  BSCON_CD,  CO_STATUS  FROM  LOG.KDS_SHOPROOM  WHERE  CO_STATUS!='07005')  A,   \n";
    ...
    private SQLObject SQLsearch_logcnt_stat_SEL2(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        ...
        query +=" AND  :END_DAY  GROUP  BY  SERIAL_NO  )  A,   \n";
        query +=" (SELECT  A.SERIAL_NO,  A.UPSO_CD,  C.BRAN_CD,  C.UPSO_NM,  C.BRAN_NM,  C.BSTYP_NM,  C.STAFF_NM,  A.BSCON_CD  FROM   \n";
        query +=" (SELECT  SERIAL_NO,  SEQ,  UPSO_CD,  CO_STATUS,  BSCON_CD  FROM  LOG.KDS_SHOPROOM  WHERE  CO_STATUS!='07005')  A,   \n";
        query +=" (SELECT  SERIAL_NO,  MAX  (SEQ)  AS  SEQ  FROM  LOG.KDS_SHOPROOM  GROUP  BY  SERIAL_NO)  B,   \n";
        query +=" (SELECT  BRAN_CD,  UPSO_CD,  UPSO_NM,  FIDU.GET_GIBU_NM  (BRAN_CD)  AS  BRAN_NM,  STAFF_CD,  FIDU.GET_STAFF_NM(STAFF_CD)  AS  STAFF_NM,   \n";
        query +=" (SELECT  GRADNM  FROM  GIBU.TBRA_BSTYPGRAD  WHERE  BSTYP_CD  =  'z'   \n";
        query +=" AND  GRAD_GBN  =  GIBU.FT_GET_BSTYP_INFO  (UPSO_CD))  AS  BSTYP_NM  FROM  GIBU.TBRA_UPSO  WHERE  BRAN_CD  =  DECODE  (:BRAN_CD,  'AL',  BRAN_CD,  :BRAN_CD))  C  WHERE  A.SERIAL_NO  =  B.SERIAL_NO   \n";
    ...
    */
    ...
    private SQLObject SQLsearch_lastlog_stat_1_SEL1(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        ...
        SQLObject sobj = new SQLObject();
        String    query="";
        query +=" SELECT  /*+  optimizer_features_enable('11.1.0.6')  */  B.UPSO_CD,  A.SERIAL_NO,   \n";
        ...
        query +=" (SELECT  CO_PARING  FROM  LOG.KDS_LOGCOLLECTOR  WHERE  SERIAL_NO  =  A.SERIAL_NO)  AS  CO_PARING,  TO_CHAR(A.REG_DATE,'YYYY-MM-DD  HH24:MI:SS')  AS  REG_DATE,  A.DIFF_DAY,  B.BRAN_NM,  B.UPSO_NM,  B.BSTYP_NM,  B.STAFF_NM,  B.BSCON_CD  FROM  (   \n";
    ...
    private SQLObject SQLsearch_lastlog_stat_1_SEL6(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        ...
        query +=" AND  BRAN_CD  =  DECODE  (:BRAN_CD,  'AL',  BRAN_CD,  :BRAN_CD)  GROUP  BY  SERIAL_NO  HAVING  CEIL(TO_DATE(:START_DAY||'235959',  'YYYYMMDDHH24MISS')  -  MAX(REG_DATE))  >=  :LOG)  A,   \n";
        query +=" (SELECT  A.SERIAL_NO,  A.UPSO_CD,  C.BRAN_CD,  C.UPSO_NM,  C.BRAN_NM,  C.BSTYP_NM,  C.STAFF_NM,  A.BSCON_CD  FROM   \n";
        query +=" (SELECT  SERIAL_NO,  SEQ,  UPSO_CD,  CO_STATUS,  BSCON_CD  FROM  LOG.KDS_SHOPROOM  )  A,   \n";
        query +=" (SELECT  SERIAL_NO,  MAX  (SEQ)  AS  SEQ  FROM  LOG.KDS_SHOPROOM  GROUP  BY  SERIAL_NO)  B,   \n";
        query +=" (SELECT  BRAN_CD,  UPSO_CD,  UPSO_NM,  FIDU.GET_GIBU_NM  (BRAN_CD)  AS  BRAN_NM,  STAFF_CD,  FIDU.GET_STAFF_NM(STAFF_CD)  AS  STAFF_NM,   \n";
        query +=" (SELECT  GRADNM  FROM  GIBU.TBRA_BSTYPGRAD  WHERE  BSTYP_CD  =  'z'   \n";
        query +=" AND  GRAD_GBN  =  GIBU.FT_GET_BSTYP_INFO  (UPSO_CD))  AS  BSTYP_NM  FROM  GIBU.TBRA_UPSO  WHERE  BRAN_CD  =  DECODE  (:BRAN_CD,  'AL',  BRAN_CD,  :BRAN_CD))  C  WHERE  A.SERIAL_NO  =  B.SERIAL_NO   \n";
