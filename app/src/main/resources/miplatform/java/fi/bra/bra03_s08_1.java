...
public class bra03_s08_1
{
    ...
    */
    public DOBJ CTLauto_demd_select(DOBJ dobj)
    {
        ...
        try
        {
            if( dobj.getRetObject("S").getRecord().get("PROC_GBN").equals("2"))
            {
                ...
                dobj  = CALLauto_demd_select_SEL13(Conn, dobj);           //  업종별 금액 합계
            ...
            else
            {
                ...
                dobj  = CALLauto_demd_select_SEL12(Conn, dobj);           //  업종별 금액 합계
    ...
    private SQLObject SQLauto_demd_select_SEL5(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        ...
        query +=" AND  A.AUTO_ACCNNUM  IS  NOT  NULL  )  XB  ,  GIBU.TBRA_UPSO  XC  ,  ACCT.TCAC_BANK  XE  ,  INSA.TCPM_DEPT  XF  WHERE  XA.DEMD_YRMN  =  :DEMD_YRMN   \n";
    ...
    // 업종별 금액 합계
    public DOBJ CALLauto_demd_select_SEL13(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        dobj.setRtnname("SEL13");
        VOBJ dvobj = dobj.getRetObject("S");           //사용자 화면에서 발생한 Object입니다.
        SQLObject  sobj = SQLauto_demd_select_SEL13(dobj, dvobj);
        VOBJ rvobj = qexe.executeQuery(Conn, sobj);
        rvobj.setName("SEL13");
        dobj.setRetObject(rvobj);
        return dobj;
    }
    private SQLObject SQLauto_demd_select_SEL13(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        String   DEMD_YRMN = dobj.getRetObject("S").getRecord().get("DEMD_YRMN");   //청구 년월
        SQLObject sobj = new SQLObject();
        ...
        query +=" AND  A.AUTO_NUM  =  B.AUTO_NUM   \n";
        query +=" AND  A.AUTO_ACCNNUM  IS  NOT  NULL  )  XB  ,  GIBU.TBRA_UPSO  XC  WHERE  XA.DEMD_YRMN  =  :DEMD_YRMN   \n";
        query +=" AND  XB.UPSO_CD  =  XA.UPSO_CD   \n";
        query +=" AND  XC.UPSO_CD  =  XA.UPSO_CD   \n";
        query +=" AND  XA.DEMD_GBN  =  '31'   \n";
        query +=" AND  XA.TOT_DEMD_AMT  >  0   \n";
    ...
    // 업종별 금액 합계
    public DOBJ CALLauto_demd_select_SEL12(Connection Conn, DOBJ dobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        ...
        dobj.setRtnname("SEL12");
        VOBJ dvobj = dobj.getRetObject("S");           //사용자 화면에서 발생한 Object입니다.
        SQLObject  sobj = SQLauto_demd_select_SEL12(dobj, dvobj);
        VOBJ rvobj = qexe.executeQuery(Conn, sobj);
        rvobj.setName("SEL12");
        dobj.setRetObject(rvobj);
        return dobj;
    }
    private SQLObject SQLauto_demd_select_SEL12(DOBJ dobj, VOBJ dvobj) throws Exception
    {
        WizUtil wutil = new WizUtil(dobj);
        String   DEMD_YRMN = dobj.getRetObject("S").getRecord().get("DEMD_YRMN");   //청구 년월
        SQLObject sobj = new SQLObject();
        ...
        query +=" SELECT  UPSO_CD  ,  MAX(AUTO_NUM)  AUTO_NUM  FROM  GIBU.TBRA_UPSO_AUTO  WHERE  TERM_YN  =  'N'  GROUP  BY  UPSO_CD  )  B  WHERE  A.UPSO_CD  =  B.UPSO_CD   \n";
        query +=" AND  A.AUTO_NUM  =  B.AUTO_NUM   \n";
        query +=" AND  A.AUTO_ACCNNUM  IS  NOT  NULL  )  XB  WHERE  XA.DEMD_YRMN  =  :DEMD_YRMN   \n";
        query +=" AND  XB.UPSO_CD  =  XA.UPSO_CD   \n";
        query +=" AND  XA.DEMD_GBN  =  '31'   \n";
